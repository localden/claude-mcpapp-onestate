...
- **Reload ordering:** `bornAt` is persisted in `localStorage` keyed by `hostContext.toolInfo.id`, so if an older widget's iframe remounts (conversation re-open, lazy-init on scroll, React remount) it reuses its *original* birth time rather than a fresh `Date.now()` that would otherwise let it steal "live" from a genuinely newer instance.
- The inline host bridge is deliberately minimal: it handles responses and notifications but does **not** reply to host→view *requests* (`ping`, `ui/resource-teardown`). Claude.ai web doesn't currently send either to views, so this is harmless for the demo. A production widget should use `@modelcontextprotocol/ext-apps` (`App` / `useApp`), which handles the full request surface.
